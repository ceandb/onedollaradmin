...
class WishlistController extends Controller
{
    ...
    public function wishlist(Request $request){
        ...
        if (empty($request->slug)) {
            request()->session()->flash('error','Producto invalido');
            return back();
        }
        ...
        if($already_wishlist) {
            request()->session()->flash('error','Ya estás en la lista de deseos');
            return back();
        }else{
        ...
        request()->session()->flash('success','Producto agregado a la lista de deseos');
        return back();
    }
    ...
    public function wishlistDelete(Request $request){
        ...
        if ($wishlist) {
            $wishlist->delete();
            request()->session()->flash('success','Producto removido de la lista');
            return back();
        }
