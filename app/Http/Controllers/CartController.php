...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        if (empty($request->slug)) {
            request()->session()->flash('error','Producto no valido');
            return back();
        }
        ...
        if($already_cart) {
            ...
            if ($cart->product->stock < $cart->quantity || $cart->product->stock <= 0) return back()->with('error','Stock no suficiente!.');
            $cart->save();
    ...
    public function singleAddToCart(Request $request){
        ...
        if($product->stock <$request->quant[1]){
            return back()->with('error','Agotado, puedes añadir otros productos.');
        }
        ...
        if($already_cart) {
            ...
            if ($already_cart->product->stock < $already_cart->quantity || $already_cart->product->stock <= 0) return back()->with('error','Stock no suficiente!.');
    ...
    public function cartDelete(Request $request){
        ...
        if ($cart) {
            $cart->delete();
            request()->session()->flash('success','Producto removido del carrito');
            return back();
        }
            ...
            foreach ($request->quant as $k=>$quant) {
                ...
                if($quant > 0 && $cart) {
                    ...
                    if($cart->product->stock < $quant){
                        request()->session()->flash('error','Fuera de stock');
                        return back();
                    }
